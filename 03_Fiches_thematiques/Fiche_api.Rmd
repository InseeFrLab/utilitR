...
Ce _package_ s'appuie sur le _package_ `apinsee` pour solliciter l'API Sirène
dans une procédure de requêtage intégrée.
À condition d'avoir renseigné les variables d'environnement décrites ci-dessus,
le _package_ permet la sollicitation de l'API Sirene, en incluant dans le paramètre `argsApi`
des éléments de paramétrage permettant la constitution de l'URL pour récupérer
en contrepartie les données désirées.
...
::: specificite
L'URL de cet API est uniquement disponible en interne à l'Insee. Il n'est pas rendu public pour des raisons de sécurité. 
:::
