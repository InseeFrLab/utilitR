...
Pour accéder à des données _via_ une API, il est recommandé de vérifier, en premier
lieu, s'il existe un _package_ `R` qui facilite l'accès aux données. 

Si l'accès par un _package_ `R` n'est pas disponible, il est recommandé d'utiliser
le _package_ `httr` pour transmettre la requête à l'API et le _package_ `jsonlite` pour retravailler le résultat de la requête afin de la transformer un `data.frame`.

Pour accéder à des données disponibles sur le site <https://www.insee.fr>,
il est recommandé
d'utiliser l'un des _packages_ suivants : `doremifasol`, `insee`, `inseeLocalData`.
Le _package_ `apinsee` permet de gérer le renouvellement des jetons de connexion. 

Si l'accès à internet se fait par l'intermédiaire d'un proxy, sous Windows, il est recommandé
d'utiliser la fonction `curl::ie_get_proxy_for_url`.
...
cachant les détails de la mise en oeuvre. Par exemple, l'API Sirene permet, à l'intérieur d'un programme de récupérer la raison sociale d'une entreprise à partir de son Sirene à partir du référentiel disponible sur Internet. 
...
L'accès à des données par des API devient de plus en plus commun et est amené
à devenir une compétence requise de tout utilisateur de données. La 
multiplication des API permet un bond en avant dans la marche vers
des traitements plus automatisés. 
En effet, les API permettent de facilement mettre à jour une base de données en
fonction de ses évolutions. Cette flexibilité accrue pour l'utilisateur permet
au producteur de données d'éviter la multiplication de duplications, avec
des versions potentiellement différentes, de sa base de données. 

Les API présentent également l'avantage, pour l'utilisateur de données, de pouvoir
facilement n'extraire qu'une partie d'une base de données plus conséquente.


### Mise à disposition des API

Les API peuvent présenter une interface utilisateur et ont toujours une documentation associée. Leur utilisation nécessite toujours de s'approprier leurs documentations.  
...
[banque de données macroéconomiques (BDM) au format SDMX (*Statistical Data and Metadata Exchange*)](https://www.insee.fr/fr/statistiques/2011033),
cette interface de requêtage interactive n'existe pas. De la documentation est mise à disposition
...
métadonnées statistiques de manière cohérente. Plusieurs institutions
internationales, dont Eurostat et l'OCDE, sponsorisent cette approche. 
...
Comme pour l'utilisation d'une fonction d'un langage de programmation,
l'appel d'une API s'effectue via des paramètres. 

Chaque service se présente sous la forme d'une URL (adresse web). Cette URL est à compléter
avec les différents paramètres de la requête.
De manière générale, l'ajout de paramètres permettant de délimiter la requête vient alimenter la constitution de l'url ; l'API va ensuite, au travers de cette url, renvoyer une réponse **structurée** contenant l'ensemble des informations demandées. 
...
S'agissant de l'API Sirene, l'utilisateur intéressé peut retrouver dans [la documentation](https://api.insee.fr/catalogue/site/themes/wso2/subthemes/insee/templates/api/documentation/download.jag?tenant=carbon.super&resourceUrl=/registry/resource/_system/governance/apimgt/applicationdata/provider/insee/Sirene/V3/documentation/files/INSEE%20Documentation%20API%20Sirene%20Services-V3.9.pdf) le paramétrage de l'url permettant de solliciter l'API.
...
n'est pas toujours facile à traiter. Certaines librairies, comme
...
des _packages_ ont été créés pour simplifier l'écriture d'une requête ou la 
...
Ce _package_ est très utile pour l'utilisation des API mises à diposition sur
...
depuis <https://api.insee.fr>. 
2. Avoir un jeton d'accès temporaire. Ces jetons ont une durée de vie
limitée et doivent régulièrement être renouvelés.

Ce _package_ propose de facilement générer un jeton à partir de `R`.
Ainsi il n'y a plus besoin de naviguer entre le programme `R`
et le catalogue des API pour renouveler un jeton. La condition est d'avoir 
renseigné, comme expliqué par la suite, les paramètres `INSEE_APP_KEY` et
`INSEE_APP_SECRET` dans un fichier de configuration. 

Pour stocker ces variables d'environnement,
il faut utiliser le fichier `.Renviron`
d'un projet. Pour cela, le _package_ `usethis` peut être utilisé:
...
::: remarque
La valeur `"user"` permet de stocker ces clés dans un fichier global, connu
...
Si, vous avez choisi la valeur `"project"` lors de
...
Ce _package_ s'appuie sur le _package_ `apinsee` pour solliciter l'API Sirène dans une procédure de requêtage intégrée. À condition d'avoir renseigné les variables d'environnement décrites ci-dessus, le _package_ permet la sollicitation de l'API Sirene, en incluant dans le paramètre `argsApi` des éléments de paramétrage permettant la constitution de l'url pour récupérer en contrepartie les données désirées.

```{r, eval = FALSE}
# Obtenir la liste des établissements créée le 1er janvier 2020
...
Ce _package_ utilise la librarie `rsmdx` qui n'est pas compatible avec
...
site de l'[OCDE](https://stats.oecd.org/index.aspx?lang=fr).
...
exemple dataframe) de `R`. L'approche par les `API` étant plus fréquente
...
- `set_config()` permet de configurer l'accés internet utilisée par les fonctions du _package_.   
...
ainsi que son indice de transformation NOVA, il faut utiliser une boucle sur
les différentes caractéristiques à extraire :
...
Le code python équivalent est un petit peu moins lourd et permet d'obtenir
un dataframe de manière plus directe. 
