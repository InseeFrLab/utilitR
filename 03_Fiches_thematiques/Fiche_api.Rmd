...
Pour utiliser cette fonctionnalité, il faut configurer l'usage du _package_ lors de la première utilisation. Plus précisément, il faut renseigner les paramètres `INSEE_APP_KEY` et
`INSEE_APP_SECRET` dans le fichier de configuration `.Renviron` de `R`. (voir la fiche [Personnaliser la configuration de `R`] pour une présenation détaillée des fichiers de configuration de `R`). Pour le faire, il suffit d'exécuter la commande suivante :

```{r, eval = FALSE}
usethis::edit_r_environ("user")
```

Votre fichier `.Renviron` est ouvert s'il existe déjà, ou s'il n'existait pas,
est automatiquement 
créé (vide), enregistré et ouvert
dans `RStudio`. Il convient d'y ajouter deux variables d'environnement, `INSEE_APP_KEY` et 
...
### Le _package_ [doremifasol](https://github.com/InseeFrLab/DoReMIFaSol)
...
```{r, eval = FALSE}
# Obtenir la liste des établissements créés le 1er janvier 2020
etablissements <- 
  telechargerDonnees(
    "SIRENE_SIRET", 
...
Cette API étant ouverte, son accès ne demande pas d'identification, ni de jeton. Il est uniquement nécessaire de déterminer les données souhaitées. Voici quelques utilisations possibles de ce _package_ :
