...
lieu, si un package `R` existe pour faciliter l'accès aux données. 

Si l'accès par un package `R` n'est pas disponible, il est recommandé d'utiliser
le package `httr` pour transmettre la requête à l'API et le package `jsonlite` pour la
...
il est recommandé
d'utiliser l'un des packages suivants: `doremifasol`, `insee`, `inseeLocalData`.
Le package `apinsee` permet de gérer le renouvellement des jetons de connexion. 
...
Nous allons voir ici quelques packages permettant de traiter l'information
...
Ce package s'appuie sur le package `apinsee` pour solliciter l'API Sirène dans une procédure de requêtage intégrée. À condition d'avoir renseigné les variables d'environnement décrites ci-dessus, le package permet la sollicitation de l'API Sirene, en incluant dans le paramètre `argsApi` des éléments de paramétrage permettant la constitution de l'url pour récupérer en contrepartie les données désirées.
...
en `Python` qu'en `R`, on trouve également plus de packages facilitant
...
Le package `httr` permet, lorsqu'on effectue une requête `GET` (une
