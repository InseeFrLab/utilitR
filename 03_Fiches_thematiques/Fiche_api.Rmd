...
le _package_ `httr` pour transmettre la requête à l'API et le _package_ `jsonlite` pour retravailler le résultat de la requête afin de la transformer dans un format plus maniable (`data.frame` ou liste de moindre profondeur).
...
pour restituer des données. Le terme d'API peut être paraître intimidant, mais il s'agit simplement d'une façon de mettre à disposition des données : plutôt que de laisser l'utilisateur consulter directement des bases de données (souvent volumineuses et complexes), l'API lui demande de formuler une requête qui est traitée par des outils informatiques spécifiques, puis lui renvoie des données en réponse à sa requête.

D'un point de vue informatique, une API est une porte d'entrée clairement identifiée par laquelle un logiciel offre des services à d'autres logiciels (ou utilisateurs). L'objectif d'une API est de fournir un point d'accès à une fonctionnalité qui soit facile à utiliser et qui masque les détails de la mise en oeuvre. Par exemple, l'API Sirene permet à l'intérieur d'un programme de récupérer la raison sociale d'une entreprise à partir de son Sirene à partir du référentiel disponible sur Internet, sans avoir à connaître tous les détails du répertoire Sirene.

À l'Insee comme ailleurs, la connexion entre les bases de données pour
les nouveaux projets tend à se réaliser via des API. L'accès à des données par des API devient ainsi de plus en plus commun et est amené
...
Une API peut souvent être utilisée de deux façons : par une interface Web, et par un logiciel. Par ailleurs, les API peuvent être proposées avec un niveau de liberté variable pour l'utilisateur :
...
Le mode principal de consultation d'une API consiste à adresser une requête à cette API _via_ un logiciel adapté (`R`, `Python`, `Java`...). Comme pour l'utilisation d'une fonction, l'appel d'une API comprend des paramètres qui sont détaillées dans la documentation de l'API. Voici les éléments importants à avoir en tête sur les r

- Le point d'entrée d'un service offert par une API se présente sous la forme d'une URL (adresse web). haque service proposé par une API a sa propre URL. Par exemple, dans le cas de l'API Sirene, l'URL à utiliser pour obtenir des informations sur un Siren est :
...
- Lorsque l'utilisateur soumet sa requête, l'API lui renvoie une réponse **structurée** contenant l'ensemble des informations demandées. Le résultat envoyé par une API est majoritairement aux formats `JSON` ou `XML` (deux formats dans lesquels les informations sont hiérarchisées de manière emboitée).
