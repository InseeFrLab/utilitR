...
Dans cette section, on présente l'utilisation basique de `duckdb`. C'est très facile car il suffit de manipuler la syntaxe `dplyr`. En particulier, il n'est pas nécessaire d'avoir des notions de SQL. 
...
```{r}
con %>% tbl("bpe_ens_2018_duckdb") %>% 
  arrow::to_arrow() %>% arrow::write_dataset("bpe_ens_2018_dataset")
list.files("bpe_ens_2018_dataset") # liste des fichiers du répertoire bpe_ens_2018_dataset/
```
...
```{r error=TRUE}
con %>% tbl("bpe_ens_2018_duckdb") |> select(DEP) |>
...
Quand on manipule des objets `duckdb`, on construit des requêtes SQL. Le package `duckdb` se contente de traduire le code `R` en `SQL` sans l'exécuter. On rappelle qu'il faut utiliser `show_query()` pour visualiser la requête. La fonction `print()` pré-visualise le résultat.
```{r}
...
```{r}
# Etape 1
res_etape1 <- con %>% tbl("bpe_ens_2018_duckdb") |>
  group_by(DEP) |>
  summarise(
    NB_EQUIP_TOT = sum(NB_EQUIP)
  ) 
...
```{r}
req <- con %>% tbl("bpe_ens_2018_duckdb") |> 
...
Pour éviter cela, il est possible d'adosser un fichier sur le disque dur à la base de données en mémoire au moment de la création de la connexion. Cela ralentit considérablement les calculs, et ne permet pas toujours d'obtenir un résultat. Dans mon expérience, il est préférable de ne pas utiliser de fichier, et d'employer les techniques présentées dans la suite de cette section.
```{r}
...
- En SQL, on peut créer un index, mais il faut que les données soient en mémoire, ce qui est incompatible avec des grosses volumétries.
...
```{r eval=FALSE}
groups <- con %>% tbl("bpe_ens_2018_duckdb") %>% 
...
- Certaines fonctions standards de `R` ne sont pas traduites, mais la situation est meilleure du côté `duckdb`.
...
-   les __empilements de tables__: il est facile d'empiler plusieurs `tibbles` avec `dplyr` grâce à la fonction `bind_rows()`: `bind_rows(table1, table2, table3, table4)`. En revanche, il n'existe pas à ce jour de fonction similaire dans `arrow`. On propose la syntaxe suivante en `duckdb`. Par ailleurs, les deux tables doivent être parfaitement compatibles pour être empilés (il faut le même nombre de colonnes avec le même nom et le même type, ce qui n'est pas toujours le cas en pratique).
    ...
    con %>% tbl("bpe_ens_2018_duckdb") %>% count()
    con %>% tbl("bpe_ens_2018_duckdb") %>% 
    union_all( con %>% tbl("bpe_ens_2018_duckdb")) %>% 
    union_all( con %>% tbl("bpe_ens_2018_duckdb")) %>% count()
    ```
