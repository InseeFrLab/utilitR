...
- Si les données sont très volumineuses (plus de 5 Go en CSV, plus de 1 Go en Parquet ou plus de 5 millions d'observations), il est essentiel de manipuler des requêtes `duckdb` plutôt que des `tibbles`, lorsque l'on réalise les traitements sur une machine standard. Les éléments présentés dans cette fiche concernent préférentiellement ce cas de figure. Si cela ne suffit pas, il faut s'orienter vers d'autres infrastructures spécialisées Big Data.
:::
