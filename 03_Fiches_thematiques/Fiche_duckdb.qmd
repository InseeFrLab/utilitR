...
De plus en plus de statisticiens utilisent la plateforme de datascience Onyxia, dont le SSPCloud et LS3 en sont des instances.
...
bpe_ens_2018_s3 <- DBI::dbGetQuery(con, glue::glue(
        " SELECT *
          FROM read_parquet('s3://{BUCKET}/XXX.parquet')
        ")
    )
```
...
bpe_ens_2018_dataset <- conn_ddb %>% tbl("read_parquet('XXX.parquet')")
