...
-   La fonction `as.POSIXct()`{.r} (**ct** pour **Calendar Time**) va
    stocker la date sous la forme d'un nombre de secondes depuis le 1er
    janvier 1970.
...
Les fonction de création de durée permettent de modifier et décaler les
dates.

Une date au format `Date` est stocké sous la forme d'un nombre de jours
ainsi :
...
(`frequency = 12L`{.r}) mais le 1er trimestre 2021 pour des séries
