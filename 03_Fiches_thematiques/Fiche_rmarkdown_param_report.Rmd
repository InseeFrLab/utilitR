...
Une fois que le modèle de rapport paramétré est défini, il est possible de l'utiliser pour publier facilement des rapports. La compilation de ce fichier, au travers de la commande `rmarkdown::render("rapportParametre.Rmd")` génèrera le document suivant :

```{r echo = FALSE, out.width = "60%"}
utilitr::include_image("./pics/rapportsparam/rapport1.png", compression = FALSE)
```

Ce rapport paramétré 

Vous pouvez noter que le paramètre `codeDpt` prend par défaut la valeur "01" (département de l'Ain).
