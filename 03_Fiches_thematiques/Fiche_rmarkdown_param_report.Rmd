...
- produire de nombreux rapports sur un même modèle en faisant varier certains paramètres.

::: {.recommandation}
Il est recommandé d'utiliser `R Markdown` pour produire ce type de rapports. Si vous ne connaissez pas `R Markdown`, il est indispensable de lire au préalable la [fiche `R Markdown`](#R Markdown).
:::
...
Dans un deuxième temps, il est souhaitable de configurer le comportement par défaut de `knitr`. La [fiche `R Markdown`](#R Markdown) <!-- lien à corriger --> détaille les principales options. Dans le cas des rapports automatisés, la configuration de `knitr` doit porter au minimum sur les deux points suivants :
...
donnees1 <- fread(file.path(dossier_donnees, "mesdonnees1.csv"))
donnees2 <- fread(file.path(dossier_donnees, "mesdonnees2.csv"))
```````
:::
...
Une fois que le modèle de rapport paramétré est défini, il est possible de l'utiliser pour publier facilement des rapports. La compilation du code source avec la commande `rmarkdown::render("rapportParametre.Rmd")` génèrera le document suivant :
