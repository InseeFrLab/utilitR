...
* Pour des tables de données de taille petite et moyenne (inférieure à 1 Go ou moins d'un million d'observations), il est recommandé d'utiliser les _packages_ `dplyr` et `tidyr` qui font l'objet de la fiche [Manipuler des tables de données avec `dplyr` et `tidyr`];
* Pour des tables de données de grande taille (plus de 1 Go ou plus d'un million d'observations), il est recommandé d'utiliser le _package_ `data.table` qui fait l'objet de la présente fiche.
:::
