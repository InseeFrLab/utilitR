...
modif_variable <- function(dt, variable = "x"){
  ...
  dt_copy[,c(variable) := get(variable) + 1] #On utilise get() lorsqu'on utilise un nom de variable avec des quotes 
  return(dt_copy)
}
```
...
CommOM: Je propose de faire un petit tableau avec le symbole `.N`, uniqueN, etc, et de donner quelques exemples.

| Fonction | Opération | Exemple     |
|----------|-----------|-------------|
|`.N`       | Nombre d'observations | `dt[,.N]` ; `dt[,.N,by = 'group_var']` |
|`uniqueN`       | Nombre d'observations unique de la variable `x` | `dt[,uniqueN(x)]` ; `dt[,uniqueN(x),by = 'group_var']` |
|`%in%`       | Nombre la liste | `dt[x %in% 1:5]` |
|`%chin%`       | Mot la liste | `dt[x %chin% 1:5]` |
|`%between%`       | Valeur entre deux nombres | `dt[x %between% c(5,13)]` |
| `%like%` | Reconnaissance d'une chaîne de caractère | `dt[departement %like% "^Haute"]`
...
Pour joindre des données, `data.table` propose une fonction `merge` plus rapide que la fonction de base. On utilisera :

| Commande | Type de jointure |
|----------|------------------|
...
Il existe d'autres manières de faire des jointures, plus poussées, présentées dans la [ici](https://rstudio-pubs-static.s3.amazonaws.com/52230_5ae0d25125b544caab32f75f0360e775.html).


::: {.conseil data-latex=""}
**Conseil**

Une fonctionalité **très puissante** pour accélérer les opérations sur les lignes (filtres, *merge*, etc. ) en `data.table` est l'**indexation**. Il s'agit de déclarer des variables faisant office de clé. C'est possible de la manière suivante: `data.table::setkey(dt, a, b))`  ou `data.table::setkeyv(dt, c("a","b"))`. Le `data.table` sera réordonné en fonction de cette variable et l'algorithme de recherche sur les lignes sera ainsi beaucoup plus efficace.

La fonction `data.table::setkey`, lorsqu'elle est appelée, met un peu de temps mais accélèrera grandement les opérations ultérieures sur les lignes. Il est vivement recommandé de l'utiliser si une ou plusieurs variables vont régulièrement servir à filtrer ou combiner des données. Pour aller plus [loin](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-keys-fast-subset.html)
:::
