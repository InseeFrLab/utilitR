...
::: {.conseil}
Modifier les options globales de data.table permet d'avoir un affichage encore un plus informatif avec les classes de chaque colonne et l'éventuel clé de la base (voir plus bas) :
```{r}
options(
...
::: {.remarque}
Cette syntaxe compacte est aussi un des atouts fondamental de data.table pour sa rapidité : au moment de faire le regroupement par z, il sait déjà qu'il n'aura que y à traiter et donc ne manipule pas les colonnes non concernées.
:::
...
- La première consiste à indiquer les colonnes à conserver sous forme de liste. La notation `.()` est un alias pour `list()` qui est pratique et concis dans un code `data.table`. Le code suivant sélectionne le code commune, le type d’équipement et le nombre d’équipement dans la base permanente des équipements :
...
|`nafill`         | Remplit les valeurs manquantes                              | `dt[,nafill(y, type="locf")]` |
...
::: {.remarque}
La fonction `.N` permet de créer facilement des compteurs avec la syntaxe `1:.N`. Par exemple `dt[ , compteur := 1:.N, by = 'group_var']`
:::
...
::: {.remarque}
La version 1.14.1 de data.table (encore en développement au 13/05/2021) apporte une syntaxe améliorée dans ce cas et considère l'utilisation de get comme désuète. Le `[...]` admet un nouvel argument `env` à qui on donne tous les remplacements que l'on souhaite. L'exemple devient :
...
L'avantage c'est qu'on peut effectuer de tels remplacements dans i et qu'on peut même remplacer des fonctions :
