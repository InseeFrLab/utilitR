...
Le *package* `data.table` permet de réorganiser facilement une table de données avec les fonctions `dcast()` et `melt()`. L'utilisation de ces fonctions est illustrée avec la table `mtcars`, transformée en `data.table`.
...
La fonction `melt()` réorganise les donnée dans un format large.
...
#### Réorganiser les données en format long (_Long to wide_)

La fonction `dcast()` réorganise les 
