...
**A première vue, on peut penser que l'opérateur `:=` est l'équivalent de la fonction `dplyr::mutate()` dans la grammaire `data.table`.** C'est en partie vrai, mais il faut garder en tête que son fonctionnement est complètement différent. En effet, **la grande spécificité de `data.table` par rapport à `dplyr` ou à `Base R` est que l'utilisation de l'opérateur `:=` modifie directement la table de données**, car  `data.table` fonctionne sur le principe de la modification par référence (pour plus de détails, vous pouvez consulter [ce lien](https://gitlab.com/linogaliana/bigr/-/blob/master/04-datatable.Rmd)). Cela signifie en pratique qu'**il ne faut pas réassigner l'objet lorsqu'on modifie une de ses colonnes**. On écrit: `dt[ , nouvelle_colonne := une_formule]` et non `dt <- dt[ , nouvelle_colonne := une_formule]`. Voici un exemple qui compare `dplyr` et `data.table`:
...
Par exemple, pour créer une nouvelle colonne, on adopte la syntaxe suivante:
