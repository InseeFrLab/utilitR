...
**Toutes les opérations précédentes pouvaient être réalisées par groupe.** il suffit d'ajouter le nom des variables de groupe dans `by` (c'est l'équivalent du `dplyr::group_by`). Lorsqu'il y a plusieurs variables de groupe, il faut écrire `by = .(var1, var2, var3)`. Par exemple, on peut calculer la moyenne de la variable `Sepal.Length` par espèce:
