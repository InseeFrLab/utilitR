...
- Le code est très concis, ce qui aide à rendre le code lisible.

Voici un exemple simple. A partir des données générées ci-dessus, on veut calculer la moyenne de `y` par groupe défini par `z`, uniquement sur les observations pour lesquelles `x` est supérieur à 3. Voici comment on peut réaliser cette opération avec `Base R`, `dplyr` et `data.table`.
...
```{r, eval = FALSE}
aggregate(dt[dt[["x"]] > 3]$y,                # Specify data column
          by = list(dt[dt[["x"]] > 3]$z),              # Specify group indicator
          FUN = sum)
```
