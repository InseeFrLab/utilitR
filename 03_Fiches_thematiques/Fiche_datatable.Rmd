...
Dans l'exemple qui suit, on réorganise la table `mtcars_dt` de façon à obtenir une table qui contient une ligne par valeur de `gear` et une colonne par valeur de `cyl`. Ces colonnes vont contenir 

La première colonne 

les colonnes `cyl` et `gear`, et comprend une ligne pour chacune des variables `mpg`, `disp`, `hp` et `wt`. Le nom et la valeur de chaque variable sont renseignées dans les colonnes `nom_de_variable` et `valeur_de_variable`.


```{r}
mtcars_dt_casted <- dcast(mtcars_dt[, cyl2 := stringr::str_c("cyl",cyl)], gear ~ cyl, value.var = "disp", fun.aggregate = mean)
...
* La fonction `dcast()` crée une colonne par n-uplet de valeurs des variables utilisées dans la partie droite de la formule. Dans l'exemple qui précède, `dcast` crée trois colonnes car la variable `cyl` prend trois valeurs (4, 6 et 8). **Il faut donc faire attention à ce que les variables utilisées dans la partie droite de la formule aient un nombre limité de valeurs**, pour ne pas obtenir une table *extrêmement* large. Pour ce faire, on peut éventuellement discrétiser les variables continues, ou regrouper les modalités avant d'utiliser `dcast()`.
* Lorsqu'on utilise `dcast()` avec une seule colonne et une seule fonction d'agrégation, on peut obtenir une table avec des noms de colonnes peu significatif si les valeurs de la variable de transposition sont numériques. On obtient des variables qui s'appellent `4`, `6`, `8` dans l'exemple précédent. Il est conseillé de modifier légèrement la partie droite de la formule pour obtenir des noms plus significatifs. Voici un exemple:

```{r}
head(mtcars_dt_casted2 <- dcast(mtcars_dt, gear ~ paste0("cyl", cyl), value.var = "disp", fun.aggregate = mean))
```
:::
