...
L'utilisation du *package* `data.table` est nettement plus complexe et déroutante pour les débutants que l'utilisation de `dplyr`. Toutefois, l'apprentissage de `data.table` est particulièrement recommandé si vous avez l'intention d'utiliser `R` avec des données volumineuses car `data.table` _beaucoup_ plus rapide et puissant que `dplyr`. Des remarques et conseils sont présents dans cette fiche pour vous aider à vous familiariser avec la syntaxe de `data.table`.
:::
