...
- Vous pouvez faire appel à d'autres fonctions à l'intérieur de la fonction `:=`. Par exemple, si on veut mettre la variable `name` en minuscules, on peut utiliser la fonction `str_to_lower` du package `stringr`. On écrit alors: 
  ```{r, eval = FALSE}
  library(stringr)
  dt[ , name_minuscule := str_to_lower(name)]
  ```
...
Le problème avec l'enchaînement d'opérations multiples est qu'on aboutit rapidement à des lignes de codes extrèmement longues. C'est pourquoi il est préférable de revenir régulièrement à la ligne. Voici 
qu'il est préférable



```{r}
ans <- iris_dt[ , `:=`(Total.Length = Sepal.Length + Petal.Length, 
                       Total.Width  = Sepal.Width  + Petal.Width)][ , .(moyenne = mean(Total.Length, na.rm = TRUE)), by = Species][order(moyenne)]
ans
```
...
**Il est important de choisir une organisation du code `data.table` qui reste lisible.** Il y a évidemment plusieurs possibilités. Voici une organisation possible: 
...
resultat <- 
  iris_dt[ ,
           `:=`(Total.Length = Sepal.Length + Petal.Length, 
                Total.Width  = Sepal.Width  + Petal.Width)
           ][ , 
              .(moyenne = mean(Total.Length, na.rm = TRUE)), by = Species
              ][
                order(moyenne)
                ]
resultat
