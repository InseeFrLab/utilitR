...
| `select(matches("...")`     | vérifie une expression régulière (_cf._ fiche [Manipuler des données textuelles](#textdata)  |
...
| Filtrer sur une variable caractère (voir la [fiche données textuelles](#textdata)) | `filter(str_detect(TYPEQU, "^A"))`      |
...
| Extraire une sous-chaine de caractères (voir la [fiche données textuelles](#textdata))  | `mutate(CATEGORIE_EQ = str_sub(TYPEQU, 1L, 1L))`                    |
