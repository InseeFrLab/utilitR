...
Un fichier `.csv` importé dans `R` (via `data.table::fread`, par exemple) peut être transformé en objet `sf` grâce à la commande `sf::st_set_geometry`. Par exemple, si les variables `lon` et `lat` contiennent des données GPS (code epsg `4326`) alors on peut faire:
...
Deux objets géométriques peuvent également être associés en fonction de leur dimension spatiale. Par exemple, les fonctions ci-dessous renvoient des `TRUE`/`FALSE` pour chaque couple de géométrie des bases `x` et `y`:
...
La fonction `sf::st_join` permet d'appliquer ces fonctions pour ne sélectionner que les conditions `TRUE` et ainsi réaliser la jointure à partir de la dimension spatiale. On utilise l'argument `left` si on désire effectuer une jointure sur la gauche (autrement un `inner join` est effectué). 
