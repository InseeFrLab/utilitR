...
Il est possible d'appliquer aux objets `sf` les fonctions de la grammaire `dplyr` comme si l'objet n'était pas un objet géographique.
...
Par exemple, imaginons que la base précédente recoupe deux groupes: les villes de plus de 2000 habitants et celles de moins de 2000 habitants. On désire faire la somme de la variable `P13_POP` par groupe. Dans ce cas, après avoir créé les groupes, on va effectuer un `group_by` puis un `summarise`:
