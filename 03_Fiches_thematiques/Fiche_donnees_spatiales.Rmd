...
**Le *package* `sf` introduit un objet particulier: la table de données spatiale appelée `sf`. Un `sf` est simplement une table de données (`data.frame`) `R` traditionnelle, enrichie d'une colonne supplémentaire `geometry` qui contient l'information géographique.**
...
CommOM: est-ce que `sf::st_read()` repère automatiquement le système de projection? Est-ce codé dans la source de données?
