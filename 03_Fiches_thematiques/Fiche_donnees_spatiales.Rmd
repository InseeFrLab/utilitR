...
Le *package* `sf` est une boîte à outils conçue pour faciliter la manipulation de données spatiales. Il repose sur le standard ISO 19125 [*simple feature access*](https://en.wikipedia.org/wiki/Simple_Features) défini conjointement par l'*Open Geospatial Consortium (OGC)* et l'*International Organization for Standardization (ISO)*. **La grande force de `sf` est qu'il permet d'exploiter des données spatiales comme s'il s'agissait de données traditionnelles.** Cette fiche illustre l'usage de `sf` avec le jeu de données `martinique` du package `cartography`.
