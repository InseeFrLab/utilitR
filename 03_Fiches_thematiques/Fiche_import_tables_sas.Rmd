...
Importer des tables SAS avec `R` est une opération souvent délicate et source d'erreurs. **Il est fortement recommandé de tester la méthode d'importation sur un petit échantillon de données avant d'importer les données**, en particulier lorsque celles-ci sont volumineuses.
...
La fonction `read_sas()` de `haven` ne fonctionne pas dans toutes les situations. Voici donc une seconde méthode qui consiste à procéder en deux temps:

* Exporter les données SAS en format `.csv`;
* Importer en R les données `.csv`.

Cette méthode propose un haut niveau de fiabilité et de performance. Elle a toutefois l'inconvénient de nécessiter un espace de stockage pour les données intermédiaires en format `csv`. 
