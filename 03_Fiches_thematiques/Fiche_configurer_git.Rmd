...
L'authentification `HTTPS` implique en principe de renseigner ses identifiants (login et
password, ou token) à chaque interaction avec le dépôt distant. Toutefois, `Git` propose
un outil pour conserver temporairement en mémoire (*cache*) des informations
d'authentification. **Cette mise en mémoire est complètement optionnelle, et vous pouvez très bien utiliser  `Git` quotidiennement sans vous servir de cette possibilité.**
