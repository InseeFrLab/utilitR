...
On peut noter que le type des variables est conservé.
...
```
La 1ère ligne est donc considérée comme une ligne de données comme les autres et les colonnes snt typées "character".
...
Il est parfois utile de pouvoir n'importer qu'une partie des données.

* **Ne pas importer les x premières lignes -**   
Le paramètre `skip` permet de préciser à compter de quelle ligne commencer l'import.  
...
head(mesDonnees,3)
```
On n'importe les données qu'à compter de la 5ème ligne. Si on laisse le paramètre `col_names = TRUE`, la 1ère de ces lignes est considérée comme *noms de colonnes*. Si ça ne doit pas être le cas, on passe le paramètre `col_names=FALSE`.

* **Importer une zone spécifique du fichier -** `range`  
Il est possible de n'importer qu'une plage de cellules en la définissant tel que cela se fait dans *Calc* i.e. en définissant les colonnes et lignes de début et de fin de la plage. **Exemple : "B2:D7"**. On peut également préciser l'onglet avant : **"Sheet1!B2:D7"**
...
Rstudio propose une interface graphique très commode pour lire, entre autres, des fichiers `xls(x)` (mais aussi des tables `Sas` ou des fichiers `csv` mais pas les `ods`). R utilise la fonction `read_excel` du _package_ `readxl` qui peut lire aisément des fichiers de type `xls` ou `xlsx` peu volumineux.  
...
Dans ce 1er exemple, on a importé l'onglet par défaut (`Sheet`), i.e. le 1er, de la table **mes_donnees.xlsx** (`File/Url`) qu'on a nommé **mes_donnees** (`Name`). On a gardé la 1ère ligne du fichier comme **noms de colones** (`First Row as Names`).
...
mesDonnees <- openxlsx::read.xlsx(xlsxFile = chemin_xlsx,  sheet="Sheet3", detectDates=TRUE)
# Structure des données
str(mesDonnees)
```
