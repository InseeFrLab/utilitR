...
L'importation de fichiers `xlsx` et `xls` va être illustré à partir de deux jeux de données créés à partir de données présentes sur le site de l'Insee, en formats `xls` et `xlsx`.  
...
download.file(telechargement_xlsx,destfile = chemin_xlsx)
download.file(telechargement_xls,destfile = chemin_xls)
```
...
download.file(telechargement_ods,destfile = chemin_ods)
```
