...
À notre connaissance, seul le _package_ `readODS` propose des fonctions propres à la lecture de fichiers `.ods` : il a 2 fonctions d'import : `read_ods` et `read.ods`.  
La documentation du _package_ recommande d'utiliser, autant que possible, `read_ods`.
...
- Adapter en conséquence le chemin ci-dessous en remplaçant `../import_donnees_tabulees_tests` par votre chemin menant au fichier `/mes_donnees.ods`. **Exemple :** `D:/T19QHL/Mes Documents/R_workSpace/documentationR/mes_donnees`.
...
On peutt noter que le type des variables est conservé.
...
Si on précise `col_types=NA`, on récupèrera un bloc de données avec toutes les colonnes typées "caractères".  
...
Le prochain exemple permet de ne pas importer la variable `Adresse` et de changer le type des variables `Département` et `Code Postal` en, respectivement, `entier` et `caractère`.  
*Nota* : les noms de variables ayant des caractères de type *espace* ou *accent* sont appelées ainsi : `` `nom de la variable` `` (`Alt Gr`+ `7`).
