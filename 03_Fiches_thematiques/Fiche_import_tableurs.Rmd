...
### Comment utiliser le _package_ `readODS`
...
L'usage de cette fonction va être illustré à partir de deux jeux de données créés à partir de données présentes sur le site de l'Insee, en formats `xls` et `xlsx`. Pour reproduire les exemples ci-dessous, vous devez:

- télécharger [les jeu de données ici (LIEU DE STOCKAGE A DEFINIR)](inactif) puis le sauvegarder sur votre poste;
- définir dans `R` les chemins des fichiers nommés `chemin_xls` et `chemin_xlsx`. Voici un exemple:
  ```{r, eval=FALSE}
  chemin_xls  <- "D:/mon_IDEP_Insee/Dossier_UtilitR/mes_donnees/mes_donnees.xls"
  chemin_xlsx <- "D:/mon_IDEP_Insee/Dossier_UtilitR/mes_donnees/mes_donnees.xlsx"
  ```
...
Rstudio propose une interface graphique très commode pour lire des fichiers `xls` et `xlsx` (mais aussi des tables `Sas` ou des fichiers `csv`, mais pas les `ods`). Le grand intérêt de cette interface est qu'elle fournit le code utilisé pour importer les données. Vous pouvez donc le copier dans vos scripts pour le réutiliser, et ainsi vous familiariser avec les fonctions d'importation. 
...
Pour importer un fichier au format `xlsx`, il est recommandé d'utiliser la fonction `read.xlsx()` du *package* `openxlsx`, en particulier si le fichier est volumineux. Cette fonction permet de charger les données du tableur dans un `data.frame`. 
...
Pour importer un fichier au format `xls`, vous pouvez utiliser la *fonction* `read_excel()` du *package* `readxl`. Cette fonction permet également d'importer un fichier au format `xlsx` peu volumineux. Si vous voulez importer un fichier `xlsx` volumineux, il est préférable d'utiliser `openxlsx`.
