...
- **Il est recommandé d'utiliser la fonction `read_ods` du _package_ `readODS` importer des fichiers `odt`.**
...
**Différences entre les formats `xls` et `xlsx`**

Les formats `xls` et `xlsx` sont deux formats de données associés au logiciel Microsoft Excel. Le format `xls` est le format historique d'Excel, tandis que le format `xlsx` a été introduit par Excel 2007. Ces deux formats présentent de multiples différences:

* Dans un classeur `xls`, la taille des tables est limitée à 65 536 lignes et 256 colonnes.
* Dans un classeur `xlsx`, la taille des tables est limitée à 1 048 576 lignes et 16 384 colonnes.
* La structure interne des fichiers `xls` et `xlsx` est très différente. Un même fichier enregistré au format `xlsx` est plus léger que s'il est enregistré au format `xls`.
* Les versions récentes d'Excel (à partir d'Excel 2007) peuvent lire les deux formats, tandis que les  versions d'Excel plus anciennes ne lisent que le format `xlsx`.

:::


**Quel _package_ utiliser pour importer son fichier `xlsx`, `xls` ou `ods` ?**  
Nous avons recensé 3 _packages_ pour l'import de fichiers avec une extension de type `xls` ou `xlsx` :

* `readxl`,
* `openxlsx`,
* `xlsx`.

Les 2 premiers _packages_ font partie des _packages_ les plus chargées sur le site du **CRAN** et répondent à nos principaux besoins. Nous excluons donc, de ce chapitre, le _package_ `xls` (qui, par ailleurs, reste intéressant pour ces fonctions d'export).

**Importer un `.xlsx`**  
Nous présenterons le _package_ `openxlsx` que nous préconisons pour les fichiers de type `xlsx` et, particulièrement, lorsqu'ils sont lourds (plusieurs dizaines de MégOctets) car : 

* il est plus plus performant : il importe un fichier de 150 MO 4 à 5 fois plus vite qu'avec le _package_ `readxl` *(3 minutes* Vs *15 minutes)*;  
...
Pour les fichiers `xls`, nous préconisons d'utiliser le _package_ `readXl` qui comporte 3 fonctions d'import :
...
À notre connaissance, seul le _package_ `readODS` propose des fonctions propres à la lecture `.ods`. Il a 2 fonctions d'import : `read_ods` et `read.ods`.
La documentation du _package_ recommande d'utiliser `read_ods`.
...
Pour lire un fichier au format `xlsx`, nous préconisons d'utiliser la *fonction* `read.xlsx()` du *package* `openxlsx`.  Cette fiche s'appuie sur la version la plus récente du _package_ : la **4.1.5**.  
...
### Pas à pas
* **Installer et charger le _package_**  
Le cas échéant, vous devrez commencer par **installer** puis **charger** le _package_ `openxlsx`
```{r ,eval=FALSE}
...
* **Installer et charger le _package_**  
Le cas échéant, vous devrez commencer par **installer** puis **charger** le _package_ `readxl`
```{r, eval=FALSE}
...
## Import d'un fichier (`xls` ou `xlsx`) avec l'interface graphique de RStudio
...
## Import d'un fichier tabulé avec le _package_ `readODS`
...
- Faire une copie du fichier ayant servi aux tests des _packages_ `xlsx` ou charger [le jeu de données ici](https://bureautique-afij.jimdofree.com/app/download/11922928825/EXERCICES+DU+COURS+BDD.xlsx?t=1487924250) puis le sauvegarder, par simplicité, sur votre lecteur **Z:/**. ;
...
Le cas échéant, vous devrez commencer par **installer** puis **charger** le _package_ `readODS`
```{r, eval = FALSE}
...
On peut également spécifié le type de différentes variables. Pour plus d'informations, se référer à `vignette("readr")`
...
*Nota* : les noms de variables ayant des caractères spéciaux, type estpace ou accent, sont appelées entre ```` (`Alt Gr`+ `7`).
...
Au contraire des fonctions d'import de fichiers `xlsx` ou `xls`, la 1ère ligne du fichier est considérée comme l'en-tête de colonnes et non comme données à proprement parlé : elle ne fait donc pas partie de la plage considérée. **Exemple : B2:D7** débutera la sélection à la 2ème ligne des données soit la 3ème ligne du fichier. 
