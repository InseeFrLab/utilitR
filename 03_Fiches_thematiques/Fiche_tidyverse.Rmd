...
Lorsqu'une suite d'opérations est amenée à être effectuée plusieurs fois, une bonne pratique consiste à *factoriser* son code, notamment grâce à des fonctions. Le site internet [pratiques.utilitr.org](https://www.pratiques.utilitr.org/) présente notamment quelques recommandations de cet ordre.

### Présentation de la difficulté à fonctionnariser

La fonctionnarisation n'est cependant pas si aisée lorsqu'on travaille avec le `tidyverse`, en raison de l'*évaluation non standard* que celui-ci exploite. En effet, le `tidyverse`est un paradigme de traitement des données qui cherche à faciliter au maximum son utilisation. Pour ce faire, comme vu précédemment, il n'est pas besoin d'écrire le nom des variables entre guillemets, ou de répeter le nom de la base que l'on exploite, ni même d'utiliser le `$`. Toutefois, cette facilité génère des problèmes lorsqu'on souhaite créer une fonction. Le code suivant montre deux traitements qui, en toute logique, devrait donner des résultats similaires.

```{r,eval=FALSE,message=FALSE}
head(bpe_ens_2018) %>% select(REG) # fonctionne

je_selectionne <- function(donnees,colonne) {
  retour <- donnees %>%
    select(colonne)
...
je_selectionne(donnees=head(bpe_ens_2018),
               colonne = REG) # ne fonctionne pas
```
Alors que `R` parvient parfaitement à comprendre que le terme `REG` fait référence à un nom de colonne de la base dans le premier cas, il ne le comprend pas dans le second cas ! Ceci est dû aux fonctions du `tidyverse` qui permettent cette flexibilité,  tandis que la fonction `je_selectionne` cherche un objet qui s'intitulerait `REG`, sans comprendre qu'il s'agit du nom de la colonne du jeu de donnée.

::: {.remarque data-latex=""}
L'évaluation non-standard est un concept complexe et très riche dans `R`. La *tiy-evaluation* en est une utilisation spécifique.

De nombreuses fonctionnalités élémentaires de R utilisent de telles évaluations. Par exemple, les commandes `library(dplyr)` ou `library("dplyr")` fonctionnent toutes les deux indiféremment. Pour autant l'objet `dplyr` de la première expression n'existe pas. `R` comprend que nous voulions dire et charge la librairie nommée `"dplyr"`.

L'évaluation, standard ou non-standard, repose donc sur l'idée que `R` doit savoir quand nous faisons référence au nom d'un objet ou à l'objet lui-même. Par exemple, quand on écrit `head(bpe_ens_2018)`, `R` comprend que l'appel à `bpe_ens_2018` fait référence à la base de données de la bpe, pas au nom en question. La *tidy-evaluation* permet de faire des appels un peu plus complexes. Ainsi, lorsque nous écrivons `select(REG)`, `R` comprend que quelques opérations préalables sont nécessaires :

1. *patienter* : il ne faut pas chercher à comprendre immédiatement le sens de `REG` car aucun objet dans notre environnement ne s'intitule `REG` ;
...
Pour réussir à créer des fonctions facilement compatibles avec le `tidyverse`, il faut faire appel à un opérateur spécifique, appelé *curly-curly* ou *doublestache* : `{{`. Son utilisation est particulièrement simple, car il suffit d'entourer la variable par ces accolades.
...
::: {.remarque data-latex=""}
Certains conseils sur internet présentent `eval(parse())`. Il est fortement recommandé de ne _jamais_ l'utiliser pour les raisons suivantes :

* il est susceptible d'induire des problèmes de sécurité. Tout le contenu à l'intérieur du `parse` est interprété par `R` comme du code à lancer. Il est donc particulièrement sensible aux problèmes d'[injection de code](https://fr.wikipedia.org/wiki/Injection_de_code_dans_les_applications_web) ;
* sa performance est très mauvaise ;
* l'identification des *bug* est particulièrement délicate, avec des messages d'erreurs peu opportuns.
...
```{r,eval=TRUE}
je_selectionne_et_renomme <- function(donnees,colonne, nouveau_nom = "nouveau_nom") {
  retour <- donnees %>%
    select({{colonne}}) %>%
...
je_selectionne_et_renomme(donnees=head(bpe_ens_2018),
               colonne = REG,
               nouveau_nom = "region") # fonctionne !
```
...
Pour fonctionnariser un code utilisant le `tidyverse`, il est recommandé :
...
Si les exemples précédents se basent tous sur `dplyr`, il est à remarquer que les solutions présentées s'appliquent à toute la philosophie du *tidyverse*, y-compris `ggplot2` (_c.f._ [Faire des graphiques avec `ggplot2`).
