...
-   soit les déclarer dans un fichier `DESCRIPTION` situé à la racine de son
    projet et installer les dépendances à l'aide la fonction
    `remotes::install_deps()` ;
-   soit utiliser le *package* `{renv}` et installer les dépendances à l'aide de
    la fonction `renv::restore()`.
...
Le fichier `DESCRIPTION` est un fichier à l'origine conçu pour renseigner les
...
projet mais aussi d'autre méta-données telles que les auteurs des programmes, la
date de réalisation, une description textuelle du projet, etc.
