...
::: {.recommandation}
La bonne méthode pour gérer les dépendances consiste à décrire explicitement les pré-requis à la bonne exécution des programmes dans un endroit prévu pour cela (approche déclarative). Cette façon de procéder permet de séparer la construction de l'environnement d'exécution du code de son exécution proprement dite, en en laissant la maîtrise à l'utilisateur. 

Inversement, forcer l'installation de dépendances en utilisant la commande `install.packages()` directement dans les codes (approche impérative) est une mauvaise méthode car elle oblige l'utilisateur du projet à modifier sa configuration sans avoir recueilli son accord. Cette approche impérative risque par ailleurs de créer des problèmes dans les autres projets de l'utilisateur.
...
-   déclarer les dépendances dans un fichier `DESCRIPTION` situé à la racine de son
