...
Le _package_ `esquisse` fournit une interface graphique qui facilite la construction de graphiques avec `ggplot2`. L'objectif de ce *package* est de vous aider à écrire rapidement un code `ggplot2` pour faire le graphique que vous voulez. En revanche, **il ne faut pas utiliser `esquisse` pour réaliser un graphique sans sauvegarder le code qui le produit** (sinon vous ne pourrez ni reproduire ni modifier votre graphique par la suite).
