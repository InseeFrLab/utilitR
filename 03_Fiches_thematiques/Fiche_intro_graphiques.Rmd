...
Le _package_ `esquisse` fournit une interface graphique qui facilite la construction de graphiques avec `ggplot2`. Pour l'utiliser, il faut évidemment préalablement installer le *package*:

```{r eval = FALSE}
install.packages("esquisse")
```

Pour lancer l'interface, ouvrez le menu *Addins* dans la barre d'outils de RStudio, et cliquez sur *'ggplot2' builder*^[Vous pouvez aussi lancer la commande `esquisse::esquisser()` dans la Console.]. Vous pouvez en apprendre davantage sur les *add-ins* dans le paragraphe de [présentation des _add-ins_](#presentation_addin).
