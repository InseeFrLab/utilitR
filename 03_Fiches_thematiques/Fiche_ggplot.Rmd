...
::: {.conseil data-latex=""}
Lorsqu'on utilise plusieurs fois le même _mapping_ dans plusieurs *geometries*, c'est-à-dire les mêmes relations entre les variables mais sous des formes différents (lignes et points par exemple), il est possible de déclarer le _mapping_ dans l’appel à `ggplot()` plutôt que de le répéter dans chaque *geometry*. Le _mapping_ sera alors valable pour toutes les *geometries* du graphique (sauf si celles-ci redéfinissent explicitement le *mapping*). Le code suivant produit exactement le même graphique que l'exemple précédent:
