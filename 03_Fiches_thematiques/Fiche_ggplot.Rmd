...
L'utilisateur souhaite réaliser des graphiques avec `R` (nuages de points, histogrammes, densité...) et les personnaliser (légendes, titres, échelles...).
...
::: {.conseil data-latex=""}
Il peut arriver qu'on utilise plusieurs fois le même _mapping_ dans plusieurs formes géométriques, c'est-à-dire qu'on définit les mêmes relations entre les variables et les attributs graphiques, mais sous des formes différents (lignes et points par exemple). Dans ce cas, il est possible de déclarer le _mapping_ directmement dans l’appel à `ggplot()` plutôt que de le répéter dans chaque forme géométrique. Le _mapping_ sera alors valable pour toutes les formes géométriques du graphique (sauf si celles-ci redéfinissent explicitement le _mapping_). Le code suivant produit exactement le même graphique que l'exemple précédent:
