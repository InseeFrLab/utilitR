...
- Pour un même fichier, il est recommandé d'utiliser le format **Parquet** qui est moins gourmand en termes de stockage que la version csv.  
- Le **package** [`arrow`](https://arrow.apache.org/docs/r/) permet de manipuler simplement les fichiers au format **Parquet** avec R.  
...
Cette fiche n'a pas vocation à être exhaustive sur le format Parquet. Elle liste ci-dessous les points saillants à retenir lorsqu'un statisticien souhaite travailler avec ce format de fichier.  
...
L'un des **avantages de stocker des métadonnées** est que cela permet au producteur d'un fichier **Parquet** de fixer le type de certaines variables dont la nature peut être ambiguë. C'est notamment le cas du numéro de département dans la nomenclature française. Comme cette variable ressemble à une valeur numérique, inférer la nature de la variable à partir des premières observations comme c'est nécessaire lors de la lecture d'un CSV peut amener à des erreurs. En effet, comment tenir compte des 0 avant certains numéros de département ou codes communes ou encore comment tenir compte des départements "2A" ou "2B" dans une variable numérique ? Avec le format Parquet, c'est le producteur du fichier, qui est généralement plus expert que l'utilisateur, qui gère ce type de cas problématiques.
:::
