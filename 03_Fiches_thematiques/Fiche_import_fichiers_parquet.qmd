...
Le monde de la data-science a été bouleversé ces derniers mois par l'arrivée d'un nouveau format de données : **Parquet**. 
...
Même si Parquet n'est pas nouveau (2013), son utilisation s'est démocratisée grâce au projet open-source [Apache](https://apache.org/) et 
plus particulièrement le projet [Apache arrow](https://arrow.apache.org/).  
...
**Parquet présente quelques propriétés qui le distinge des formats de fichiers plus populaires :**
...
Le package `arrow` présente une fonctionnalité supplémentaire qui consiste à créer un fichier **Parquet partitionné**. Partitionner un fichier revient à le "découper" selon une clé de partitionnement (qui peut prendre la forme par exemple d'une ou de plusieurs variables). Cela permet de pouvoir exécuter du code sur une table volumineuse qui dépasse la mémoire de son espace de travail dans la mesure où les requêtes seront alors exécutées selon **un plan d'exécution optimal**.
...
**Bonus :** avec le package [`parquetize`](https://cran.r-project.org/web/packages/parquetize/index.html), il est possible d'exécuter l'équivalent du code précédent à l'aide de la fonction [csv_to_parquet()](https://ddotta.github.io/parquetize/reference/csv_to_parquet.html).
