...
La fonction `str_squish()` permet de supprimer tous les espaces aux extrémités ou en double à l'intérieur d'une chaîne de caractères. 
...
* L'expression régulière `"([0-9]{2}\\.*){5}"` permet de rechecher cinq séries de deux chiffres éventuellement séparées par un point. Exemple : `str_detect("06.12.87.45.13.", pattern = "([0-9]{2}\\.*){5}")`.
...
- la [partie `stringr`](https://juba.github.io/tidyverse/11-stringr.html) de l'introduction à `R` et au `tidyverse` ;
- la [vignette](https://github.com/tidyverse/stringr/blob/master/vignettes/from-base.Rmd) comparant `stringr` et `base R`.
