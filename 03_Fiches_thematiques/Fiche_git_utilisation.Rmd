...
Il est également souhaitable de lire au préalable la fiche XXXX et de vérifier que les éléments de configuration de `Git` présentés dans cette fiche sont fonctionnels.
...
La troisième étape consiste à établir un lien entre votre projet local et un dépôt distant (`git add remote`). Pour créer ce lien avec l'interface de `RStudio`, il faut se rendre dans la partie destinée à la gestion des branches sur laquelle la partie [RStudio et les branches](#git-branches) revient plus en détails.
...
* 
```{r, out.extra='style="display: inline-block; padding: 0;"', echo = FALSE}
