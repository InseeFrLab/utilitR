...
Il est possible, depuis `RStudio`, de consulter la liste des modifications apportées à un dépôt en cliquant sur le bouton `History`.

```{r, echo = FALSE}
include_image("./pics/git/onglet_git6.png")
```

Cette action ouvre une fenêtre où chaque `commit` fait sur la branche (voir plus tard) est listé, avec les modifications concernées.
