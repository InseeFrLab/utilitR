...
* Il est recommandé d'utiliser systématiquement `Git` pour suivre les modifications de tout projet RStudio qui comprend des codes `R` ;
* Il est recommandé de ne jamais utiliser `Git` pour sauvegarder les données ;
* Il est recommandé d'utiliser l'interface graphique de `RStudio` pour les usages courants de `Git`, plutôt que la ligne de commande ;
* Vous pouvez utiliser `Git` _via_ la ligne de commande si vous le souhaitez. En revanche, il est très fortement déconseillé d'exécuter une commande qui comprendrait les termes `force` ou `rebase`.
