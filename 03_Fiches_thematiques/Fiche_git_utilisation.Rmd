...
RStudio permet d'utiliser `Git` _via_ une interface graphique. Cette interface est accessible dans l'onglet `Git` situé en haut à droite de la fenêtre RStudio. C'est grâce à cet onglet que vous pourrez effectuer la plupart des opérations `Git` courantes, qui sont présentées ci-dessous.
...
::: {.remarque}
Si l'onglet `Git` n'apparaît pas, c'est que votre projet RStudio n'utilise pas `Git`. Reportez-vous à la section [].
:::
...
Supposons qu'un fichier source a été modifié et qu'on désire valider ces modifications. Ci-dessous, le dépôt comporte plusieurs fichiers modifiés.
