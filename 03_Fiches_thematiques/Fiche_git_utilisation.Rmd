...
travail en local. Ceci est développé dans la
...
`R` la commande `usethis::use_git`. Ceci est développé dans la Section
...
* Il est recommandé d'utiliser RStudio pour créer une branche ou naviguer entre
les branches ;
* Il est recommandé d'utiliser les interfaces `Github` ou `Gitlab` pour
faire un `merge`. A cet égard, voir la
...
* Pour les usages courants de `Git`, l'interface graphique de `RStudio` est 
à privilégier. 
* Pour les usages avancés ou pour les utilisateurs plus experts, la ligne de
commande peut être utilisée. Certaines commandes sont néanmoins à proscrire,
notamment celles impliquant les termes `force` ou `rebase`
...
La réponse à toutes ces questions est : __utiliser `Git`__. `Git` est un
outil qui permet de suivre en détail les évolutions d'un projet impliquant
du code informatique, et qui facilite l'archivage des codes et la
...
## Utiliser `Git` avec `R` dans un projet RStudio {#clone}

Il est possible de connecter un projet `Rstudio` à tout moment avec `Git`, que
ce soit à la création de celui-ci ou, si cela n'a pas été fait lors de la
création, plus tard


### A l'initialisation d'un projet {#clone-canonical}
...
Certains fichiers doivent être exclus du contrôle de version. C'est le cas,
par exemple, de fichiers de données ou de fichiers stockant des informations
confidentielles.

Les fichiers exclus du suivi de version sont répertoriés dans un fichier 
nommé `.gitignore`. Si le projet a été créé selon les méthodes précédement
évoquées, `Rstudio` crée automatiquement ce fichier `.gitignore` contenant
...
Ce fichier est éditable et il est souvent nécessaire de lui ajouter quelques
lignes pour exclure un certain nombre de fichiers. Par exemple, pour exclure
les formats de données csv, SAS et Excel, les lignes suivantes peuvent être
ajoutées au fichier
...
Il est également recommandé d'exclure les fichiers pouvant être produits par
des documents `R Markdown`. Par exemple, pour exclure les fichiers `PDF` et
`HTML`, les lignes suivantes suffisent
...
::: {.conseil}
Le site [https://www.toptal.com/developers/gitignore](gitignore.io) fournit
un certain nombre de modèles de fichiers `.gitignore` selon le type de projets
associé à `Git`. 
:::
