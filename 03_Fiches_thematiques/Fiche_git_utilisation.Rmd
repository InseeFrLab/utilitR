...
::: {.remarque}
Cette fiche décrit l'utilisation de `Git` au travers de l'interface graphique RStudio, car celle-ci facilite l'apprentissage de `Git` pour les débutants. Toutefois, utiliser cette interface n'est nullement obligatoire, et il est possible de réaliser l'ensemble des gestes présentés dans cette fiche avec des lignes de commande.

Pour les utilisateurs intéressés par une telle approche, le nom de la commande `git` concerné sera systématiquement mentionné.
:::

## Initialiser l'usage de `Git` dans un projet RStudio {#clone}

Il est possible de commencer à suivre les modifications d'un projet `RStudio` à tout moment avec `Git`. **Il est néanmoins nettement préférable de le faire dès la création du projet**, mais il est possible de le faire plus tard, à n'importe quelle étape du projet.
...
avec vos collègues ou stocker vos codes personnels.

Que ce soit avec `Gitlab` ou `Github`, les fonctionalités pour créer un
nouveau dépôt sont très similaires. Dans les deux cas, après avoir choisi
le nom du dépôt, il est possible de choisir la visibilité de celui-ci (privé
ou public), le créer vide ou avec un fichier README minimal ou suivre un
modèle de dépôt (*template*) particulier
(cette fonctionalité est plutôt là pour aider à la création de gros
projets informatiques traditionnels
que pour des projets statistiques). La seule différence entre les deux 
plateformes est dans la dénomination (`repository` sur `Github`, `project` sur
`Gitlab`) et à l'endroit où il est nécessaire de cliquer pour créer un dépôt:

* Sur `Gitlab`, il suffit de cliquer sur `New project` dans la partie supérieure
de la fenêtre:
...
* Sur `Github`, il suffit de cliquer, en haut à droite, sur
`New repository`: 
...
Si le dépôt est créé sans `README`, `Gitlab` comme `Github` proposent quelques
lignes de commande pour connecter ce dépôt à son poste personnel. 
Les méthodes proposées par la suite permettent de se passer de la ligne de
commande tout en donnant un résultat équivalent.


Deux points sont importants sur la création du dépôt distant :
...
#### Etape 3 : Mettre en place le _remote_ (`git add remote`)

La troisième étape consiste à établir un lien entre votre projet local et un dépôt distant. 
...
* `Remote name` : le nom du dépôt distant. Par convention, l'alias est généralement nommé `origin` (mais vous pouvez lui donner un autre nom) ;
...
Les modifications apportées à chaque fichier sont résumées ligne à ligne dans la partie inférieure (suppressions en rouge, ajouts en vert). Pour valider ces modifications (`commit`), il faut écrire un message qui décrit la nature des modifications sauvegardées (dans le cadre vert) puis cliquer sur le
bouton `r with_def("commit")` (ovale rouge). Une fenêtre s'ouvre alors et confirme que la modification a été sauvegardée :
