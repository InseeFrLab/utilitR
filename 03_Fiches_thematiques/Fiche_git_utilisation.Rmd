...
un dépôt distant (`git add remote`). Pour créer ce lien avec
...
::: {.remarque}
Les icônes de l'onglet `Git` sont placées dans l'ordre des actions à réaliser afin de publier un code sur un dépôt distant : (add), `with_def("commit")`, pull, push.
...
- Les modifications faites sur les codes dans une `r with_def("branche")` n'altèrent pas la version de référence (celle de la `r with_def("branche")` `master`) tant qu'il n'y a pas de fusion entre `r with_def("branche")` (`merge`).
