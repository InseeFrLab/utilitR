...
effectuer les manipulations de la Section XX et XX qui s'effectuent sur une
copie locale (`clone`) du dépôt.

Néanmoins, il sera impossible d'effectuer une opération `push` car cela
nécessite des droits d'écriture sur le dépôt qui est réservée aux mainteneurs
de la documentation. Pour pratiquer ces opérations, il est recommandé de se
créer un dépôt personnel sur `Github` ou `Gitlab`.
:::
...
Cette fonction réalise deux opérations : elle crée automatiquement tous le
fichiers de configuration nécessaires au bon fonctionnement
de `Git` dans le projet sur lequel vous travaillez, et effectue, si vous
l'avez accepté,
un premier `commit` (voir Section XX) pour valider la création de ces fichiers.
Attention, si vous avez des fichiers qui ne doivent pas être sous contrôle de
version (des données, des codes compilés, etc.): 

* refusez de *committer* 
ces fichiers ;
* ajoutez les lignes nécessaires au `.gitignore` (voir partie
[sur le fichier .gitignore](#gitignore) pour plus de détails) ;
* valider l'ensemble des fichiers maintenant que ces fichiers ont été exclus


#### Etape 2 : Mettre en place le _remote_

La fonction `usethis::use_git` ne crée pas le lien entre le dépôt distant
et le projet local. Elle n'a vocation qu'à créer l'environnement propice
à l'utilisation, en local, de `Git`. 
...
l'interface `Rstudio`, il faut se rendre dans la partie destinée à la gestion
des branches sur laquelle la partie [RStudio et les branches](#git-branches)
revient plus amplement.
...
* `Remote name`: l'alias du dépôt distant. Par convention, l'alias est généralement
nommé `origin`
* `Remote URL`: l'url du dépôt distant (voir plus haut)

En ligne de commande, la manière équivalente de faire cela serait de 
faire:

```shell
git add remote origin https://MON_URL.git
```

A ce stade, `Git` est maintenant capable de contrôler les versions des fichiers
locaux et d'interagir avec le dépôt. L'environnement nécessaire pour
pouvoir réaliser les opérations usuelles, présentées dans la partie
[Suivre les modifications avec RStudio](#git-local) est ainsi opérationnel. 
...
### Sauvegarder les modifications de vos codes
