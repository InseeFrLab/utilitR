...
## Utiliser `Git` pour suivre les modifications d'un projet RStudio {#clone}
...
### Initialiser `Git` à la création d'un projet {#clone-canonical}
...
Lorsqu'on crée un projet `Rstudio` (voir fiche XXXX), choisir la troisième option (_Version Control_) :

```{r, echo = FALSE}
include_image("./pics/git/create_project_1.png")
```

Dans la fenêtre qui apparaît, coller l'url récupérée précédemment dans le premier champ :

```{r, echo = FALSE}
include_image("./pics/git/create_project_2.png")
```

Le troisième champ définit l'emplacement de votre dépôt local dans l'aborescence  de votre poste. Il est possible de la modifier en cliquant sur le bouton `Browse`.

Après avoir validé, `Rstudio` ouvre un projet `Rstudio` (ou le crée s'il n'existait pas dans le dépôt `Git` disponible en ligne). Un nouvel onglet, portant logiquement le nom de `Git`, apparaît en haut à droite :

```{r, echo = FALSE}
include_image("./pics/git/create_project_3.png")
```

C'est grâce à cet onglet que vous pourrez effectuer la plupart des opérations `Git` courantes, ce qui est présenté à partir de la section [Premiers pas en local](#git-local).

### Ajouter `Git` à un projet RStudio déjà existant {#clone-alternative}

Il est également possible 

Le _package_ `usethis` propose de nombreuses fonctions utiles lorsqu'on
...
La commande `usethis::use_git()` crée automatiquement tous les fichiers de configuration nécessaires au bon
fonctionnement de `Git` dans le projet sur lequel vous travaillez. Au passage,
