...
Pour pouvoir soumettre sa version de travail grâce à un `push`, il est nécessaire d'avoir un dépôt local à jour
avec la version distante. Cela assure, à tout instant, la cohérence entre les différentes copies des contributeurs d'un projet. Donc, récupérer la dernière version en ligne (`pull`) doit toujours précéder l'envoi de modifications (`push`).
