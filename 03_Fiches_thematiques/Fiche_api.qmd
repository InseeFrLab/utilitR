...
- `httr2` permet nativement de gérer la transformation des retours au format JSON, mais au besoin pour des cas complexe le _package_ `jsonlite` peut être utilisé
- Pour accéder à des données disponibles sur le site <https://www.insee.fr>,
il est recommandé d'utiliser l'un des _packages_ suivants : `doremifasol`, `melodi` (Melodi), `insee` (séries BDM). 
