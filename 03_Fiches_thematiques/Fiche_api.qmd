...
resultats <- httr2::request(url) |>                   # url à interroger
  ...
  httr2::resp_body_json(simplifyVector = FALSE)  # extraire le corps de la réponse
```
...
Par exemple en extrayant manuellement le libellé et le nutriscore d'un produit, ainsi que son indice de transformation NOVA :
...
resultats <- request(url) |>
  ...
  resp_body_json(simplifyVector = FALSE)                  # extraction JSON
```
...
<!----- moins utile depuis httr2::resp_body_json() ? ----->
