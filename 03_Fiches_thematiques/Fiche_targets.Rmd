...
Le *package* `targets` peut être particulièrement intéressante dans le cas d'un projet d'étude qui vise à une forte reproductibilité. Plus précisément, utiliser `targets` pour un projet permet de:
...
Le nouvel utilisateur qui souhaite tester `targets` crée un projet Rstudio au sein duquel il crée un fichier `_targets.R`. En prévision des futures fonctions qu'il va écrire, il crée un dossier 'R'. Le fichier `_targets.R` détaille l'enchaînement des traitements. Il doit toujours charger le *package* `targets`.
