...
La chaine de traitement est donc ici linéaire. Chaque étape dépend
...
_packages_ `dplyr` et `readr` dans notre chaine de traitement.
...
l'ensemble des étapes du traitements.
La complexité des traitements est résumé de façon concise
par un ensemble minimal de fonction résumant les grandes étapes.
...
Lorsque la chaine de traitement est de taille relativement
modeste, comme ici, on peut la
visualiser avec la fonction
`tar_visnetwork`:
...
de la chaine dans le fichier `_targets.R`. Par exemple 
...
chaine ayant la structure suivante
...
le temps nécessaire pour mettre à jour l'ensemble de la chaine
de traitement
...
emplacement dans la chaine de traitement, puisque chaque
...
En premier lieu, le format par défaut
qui peut être utilisé est le
format [qs](https://cran.r-project.org/web/packages/qs/index.html).
Celui-ci est, comme `rds`, est un format spécifique à `R` mais
présente l'avantage d'être beaucoup plus rapide et ainsi limiter
les temps en lecture/écriture.
...
- `parquet`: un format qui tend à devenir un standard dans le monde
...
- `fst_tbl` (utilisateurs du tidyverse) ou `fst_dt` (utilisateurs de data.table) : un format qui est spécifique à `R` mais présente
des avantages proches de ceux d'un fichier `parquet`. L'avantage de ces
formats est qu'ils préservent la nature d'un dataframe, ce qui permet de
...
de fichiers markdown à l'issue d'une chaine de
traitement. 

Néanmoins, deux philosophies existent pour produire un 
fichier reproductible dans une chaine de traitement :

- Intégrer directement le fichier à la chaine
comme une étape finale du processus
de production. Cela revient à produire le `RMarkdown`
via un `tar_target` particulier ;
- Exécuter la chaine de traitement, ou les parties nouvelles
de la chaine de traitement, directement depuis le fichier `RMarkdown`.
Dans ce cas, le fichier `.Rmd` n'est plus exécuté depuis le `_targets.R`
mais au contraire sert à l'exécuter.


## Concevoir un rapport en sortie de chaine de traitement
...
Par exemple, on peut écrire un Rmarkdown `report.Rmd` considéré comme une des cibles de l'analyse (par exemple, c'est le compte-rendu de l'analyse), et qui dépend d'autres cibles.
...
## Utiliser des objets issus d'une chaine de traitement dans un R Markdown

Cette méthode est particulièrement appropriée lorsqu'on 
désire prototyper un rapport en utilisant un ou plusieurs
objets de la chaine de traitement.

Plus d'éléments sont disponibles [dans la documentation officielle](https://books.ropensci.org/targets/markdown.html#target-markdown)
...
Comment créer des cibles automatiquement (sans écrire explicitement dans `_targets.R` chacune d'entre elles)? `targets` propose de décliner les cibles en _"branches"_.

On distingue :

* les branches définies dynamiquement : avant l'exécution, le nombre de branches est inconnu ;
* les branches définies statiquement : le nombre de branche est définie précisément avant l'exécution.
...
Voici un exemple minimal de pipeline qui va itérer sur _N_ couples d'arguments une même _"simulation"_, en évitant de créer _N_ cibles distinctes pour les _N_ résultats,
et plutôt créer une seule cible résultats qui donnera lieu à autant de branches
que de _"simulations"_: 
...
Ce qui distingue ici la cible `resultat`  de ce qui a été vu précédemment, c'est l'utilisation de l'argument `pattern`,
qui a vocation à itérer sur les vecteurs cibles `x`  et `y` grâce à `map`.
...
On remarque d'ailleurs que `targets` a compris que cela ne changeait pas certains résultats précédents
(3 branches strictement identiques, qui ne sont pas recalculées).
