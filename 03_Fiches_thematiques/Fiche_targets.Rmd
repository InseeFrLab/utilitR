# Une chaîne de traitement reproductible avec `targets`
...
4. Faciliter la prise en main par une autre personne par une organisation standardisée des codes et une description complète de l'enchainement des étapes intégrée dans le code lui-même


## Quelles sont les tâches automatisées par `targets`?

`targets` permets de définir et d'exécuter une chaîne de traitement avec:
...
Un projet `targets` est un projet R qui contient les principaux éléments suivants :
...
La structure du projet doit être comme suit:
...
Le nouvel utilisateur qui souhaite tester `targets` crée un projet Rstudio au sein duquel il crée un fichier `_targets.R`. En prévision des futures fonctions qu'il va écrire, il crée un dossier 'R'. Le fichier `_targets.R` détaille l'enchaînement des traitements. Il doit toujours charger la librairie `targets`.
...
Souvent, les cibles d'une analyses (étapes intermédiaires) sont nombreuses et ont un certain degré de redondance. 
...
Ce qui distingue la ici cible `resultat`  de ce qui a été vu précédement, c'est l'utilisation de l'argument "pattern", qui a vocation à itérer sur les vecteurs cibles x et y grâce à "map".
...
La librairie `tarchetypes` est un complément utile. Cette librairie permet d'intégrer simplement des rapports Rmarkdown dans la pipeline avec `tarchetypes::tar_render()`. L'essentiel des calculs doit être en amont du rapport markdown, qui doit être rapide à exécuter. 
