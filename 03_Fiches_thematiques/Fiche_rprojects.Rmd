...
- Les projets RStudio sont également très utiles pour assurer le contrôle de version avec `Git`. Les éléments relatifs à l'usage de `Git` sont présentés dans la fiche [Utiliser Git avec RStudio](#git).
...
Un programme est reproductible lorsque tout utilisateur est en mesure de faire tourner l'intégralité du programme sans erreur, et de retrouver des résultats identiques. Assurer la reproducibilité d'un programme est une démarche exigeante qui requiert de développer des codes propres, clairs et bien documentés.
...
![Créer un nouveau projet: `File > New Project`](./pics/rprojects/0_projet_minimal.png){width=50%} 
...
![Créer un nouveau projet: `File > New Project`](./pics/rprojects/02_projectnew.png){width=50%} 
...
![Différents types de projets](./pics/rprojects/02_projectnew2.png){width=50%}

### Structure d'un projet `RStudio`

La création d'un projet `RStudio` se traduit par trois actions, quel que soit le type de projet :
...
* le projet est chargé dans `RStudio` et le nom du projet est affiché dans la barre de projet (tout en haut).

Enfin, d'autres fichiers peuvent être créés automatiquement, en fonction du type de projet créé (voir section précédente).

Le menu à droite permet d'afficher les fichiers contenus dans le projet.


Ci-dessous, en bas à droite, on peut retrouver cette structure avec un fichier `.RProj` et un `.gitignore` :

![Illustration de structure d'un projet R](./pics/rprojects/03a_project_organization1.png){width=50%}


On peut accéder aux derniers projets ouverts avec le raccourci en haut à droite :

![Raccourci pour accéder aux derniers projets](./pics/rprojects/03a_project_organization2.png){width=30%}
...
:::{.conseil}
Un point important concerne le fichier `.RData`. 

Sauvegarder et recharger les données du projet avec le fichier `.RData` est une **très** mauvaise pratique en termes de reproductibilité. En effet, il se peut que les données sauvegardées dans le `.RData` aient subi des modifications non documentées (par exemple _via_ la console), ou que les instructions présentes dans les scripts du projet n'aient pas été exécutées dans l'ordre. Dans ce cas, les données contenues dans le `.RData` ne sont pas nécessairement cohérentes avec le projet, et en les utilisant vous risquez de ne pas obtenir les résultats que vous voulez. 

Il est donc fortement conseillé de désactiver la sauvegarde du `.RData` à la fermeture du projet. Pour ce faire, il faut aller dans `Tools > Global Options` et rester dans l'onglet `General`. Chercher `Save workspace to .RData on exit:`, et choisir l'option `Never`.
...
Enfin, à l'Insee, sous AUS, il est certes possible d'installer des *addins* présents sur le CRAN mais le package `addinslist` ne fonctionnera pas car il va chercher la liste des *addins* disponibles sur internet, ce qui n'est pas possible sur AUS.
...
![Addins RStudio](./pics/rprojects/06_addins.png){width=80%}

![Installer facilement de nouveaux `addins` avec `addinslist`](./pics/rprojects/06b_addins.png){width=80%}
