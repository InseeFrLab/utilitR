# Produire des documents avec `R Markdown`
...
```{r, echo = FALSE, fig.cap= "Comment R Markdown produit-il un document ?"}
include_image("./pics/rmarkdown/schema_rmd.png")
```
...
include_image("./pics/rmarkdown/knit.png")
```
...
```{r, echo = FALSE}
include_image("./pics/rmarkdown/toc.png")
```
...
```{r, echo = FALSE}
include_image("./pics/rmarkdown/chunk.png")
```
...
include_image("./pics/rmarkdown/namedbloc.png")
```
...
```{r, echo = FALSE}
include_image("./pics/rmarkdown/perso_options.png")
```
...
```{r, echo = FALSE}
render_rmd("resources/rmarkdown/chunk07.Rmd")
```
donnera l'*output* suivant :

```{r, echo = FALSE, fig.cap= "Exemple de rendu de la bibliographie"}
include_image("resources/rmarkdown/chunk07.png")
```
...
include_image("./pics/rmarkdown/googlescholar.png")
```


Cela ouvre alors un onglet vierge avec la citation en question, en l'occurence 
...
```{r echo = FALSE, comment = ""}
render_rmd("resources/rmarkdown/chunk08.Rmd")
```

### Sans passer par un fichier image

L'avantage de `R Markdown` est que la création du document peut être faite sans générer de sorties intermédiaires. Si un bloc de code propose un graphique en output, celui-ci sera automatiquement intégré au document. Par exemple, le bloc suivant permet de représenter l'histogramme des populations communales (avec une échelle logarithmique) à partir des données Filosofi agrégées, disponibles dans le _package_ doremifasolData.

```{r echo = FALSE, comment = ""}
render_rmd("resources/rmarkdown/chunk09.Rmd")
```

```{r, echo = FALSE, warning = FALSE, comment = FALSE, message = FALSE}
library(doremifasolData)
library(ggplot2)
data(filosofi_com_2016, package = "doremifasolData")
