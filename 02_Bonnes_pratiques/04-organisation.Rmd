...
## Organisation d'une série de traitements
...
1. un unique fichier `maître` qui appelle des fonctions qui ont été intégrées dans un package. Cette approche est la plus adéquate pour assurer la reproductibilité des traitements car les *packages* gèrent de manière explicite les relations entre les fonctions et environnements ainsi que la documentation. 
...
La première approche est plus adéquate pour un projet collaboratif ou un projet de chaîne de traitements statistiques (où la reproductibilité est un impératif!). La deuxième approche peut convenir pour un projet personnel ou une étude ponctuelle. Elle nécessite néanmoins une rigueur proche de la première.
...
source("01_Import_donnees.R", encoding = "UTF-8", echo = TRUE)
source("02_Traitement_donnees.R", encoding = "UTF-8", echo = TRUE)
source("03_estimation_modele.R", encoding = "UTF-8", echo = TRUE)
source("04_Sorties_tableaux.R", encoding = "UTF-8", echo = TRUE)
source("05_Sorties_graphiques.R", encoding = "UTF-8", echo = TRUE)
```

La première approche, celle fondée sur des fonctions encapsulées dans un ou des packages, constitue une approche plus fiable pour la reproductibilité du code car elle évite les erreurs, parfois difficiles à détecter, liées aux environnements et chemin de noms des packages. La transformation de scripts modularisés en *packages* a été nettement facilitée au cours des dernières années. Nous vous recommandons la dernière partie de la formation **Travail collaboratif avec R** à cet égard.
