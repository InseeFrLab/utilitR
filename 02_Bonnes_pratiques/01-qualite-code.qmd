...
qu'un développeur la concision n'utilisera pas de `return`. De même,
...
également faciliter la documentation du code en permettant de décrire clairement ce que fait chaque fonction. Il est par  ailleurs à noter qu'un nom bien choisi pour une fonction peut déjà donner une bonne idée de ce à quoi elle sert, facilitant en cela la compréhension d'une chaîne de traitements.
...
En utilisant `R`, le _package_ chargé la plus récemment via `library`
...
d'indiquer explicitement le _package_ en utilisant la notation `package::fonction()`.
