...
::: {.recommandation}
- Avant d'installer un _package_, il est indispensable de déterminer précisément quel est le _package_ dont vous avez besoin. Pour cela, vous pouvez consulter la partie 2 de la documentation `utilitR`, et la fiche [Comment choisir un _package_ ?].
...
(seules les versions stables sont en général soumises au CRAN). Pour installer
un _package_ tel qu'il était à un moment donné, il est également possible de
faire suivre le nom du dépôt par une référence à un `with_def("commit")`, un tag
ou une `with_def("branche")`.
Exemples : `install_gitlab("py_b/funprog@v-0.3.0")`.
