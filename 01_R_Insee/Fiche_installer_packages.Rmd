...
D'un point de vue technique, un _package_ est constitué d'objets `R` (la plupart du temps des fonctions, parfois des données) et d'une documentation empaquetés en un seul fichier, dans le but d'être partagé facilement.
...
Pour utiliser un _package_ `R`, il est nécessaire de réaliser deux actions distinctes, qui sont détaillées dans la suite de cette fiche :
...
Il est important de comprendre que ces deux actions sont complètement distinctes. L'installation est une opération qu'on ne réalise qu'une seule fois : une fois qu'un _package_ a été installé sur un ordinateur, il y est présent **de façon permanente**. A noter qu'il existe toutefois des raisons de réinstaller un _package_, par exemple pour le mettre à jour. Inversement, le chargement d'un _package_ est une opération qu'il faut réaliser à chaque fois vous ouvrez une session `R`.

### Version et dépendances.

UN PACKAGE R EST DEPENDANT D'UNE VERSION DE R.

LA NOTION DE DEPENDANCES


## Installer un _package_ depuis un dépôt distant
