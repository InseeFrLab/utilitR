...
- du fait qu'il n'y a pas d'accès à Internet, les _packages_ sont installés à partir d'un dépôt interne à l'Insee (synchronisé quotidiennement avec le CRAN) ;
- les _packages_ installés par un utilisateur sont stockés sur son espace personnel, dans le dossier `(ESPERT)/R/win-library` ;
- les fichiers de configuration `.Rprofile` et `.Renviron` sont recherchés au démarrage dans le dossier `(ESPERT)/R`.
