...
- [Cliquer sur l'onglet **"Catalogue de services"**](https://datalab.sspcloud.fr/my-lab/catalogue) qui est accessible à gauche grâce à ce symbole ![](./pics/SSPCloud/01_bouton_services.png#inline){width="30"}\ 

- La fenêtre suivante s'affiche (pour y accéder directement,
[suivre ce lien](https://datalab.sspcloud.fr/catalog-new/inseefrlab-helm-charts-datascience)). 

```{r lancer_rstudio, echo = FALSE, fig.cap= "Lancer un service RStudio", out.width = "100%"}
...
utilitr::include_image("./pics/SSPCloud/02_lancer_rstudio.png")
```


<!-- <img src = "pics/SSPCloud/03_bouton_mes_services.png" width="30" height="30" style= 'display: inline; vertical-align: middle'></img> -->


### Accéder à ses services

Pour accéder à l'ensemble de ses services, il est nécessaire de cliquer sur le bouton ![](./pics/SSPCloud/03_bouton_mes_services.png#inline){width="30"}\ .
...
utilitr::include_image("./pics/SSPCloud/03_liste_services.png")
```

Pour accéder à l'un d'eux il faut cliquer sur le
bouton  ![](./pics/SSPCloud/Se_connecter_service.png#inline){width="30"}\ .
...
![](./pics/SSPCloud/key.png#inline){width="30"}\ présente dans la page
*Mes services* ![](./pics/SSPCloud/03_bouton_mes_services.png#inline){width="30"}\. Cette action permet de copier le mot de passe dans le presse-papier ce qui
facilite son utilisation.

Pour accéder à un service, le bouton
![](./pics/SSPCloud/Se_connecter_service.png#inline){width="30"}\ permet d'ouvrir une page d'authentification. 
...
- L'**option `memory`** (dans l'onglet `resources`) définit la quantité maximale de mémoire vive (mesurée en Mo) que votre service  RStudio peut utiliser. **Cette option est particulièrement importante** et vous devez choisir soigneusement sa valeur : si vos traitements requièrent plus de mémoire que la valeur de `mem`, la seule solution consiste à lancer un autre service  RStudio avec une valeur de `mem` plus élevée. Il est conseillé d'utiliser 10000 comme valeur par défaut.
- L'**option `cpu`** (dans l'onglet `service`) définit le nombre minimal de processeurs alloués à votre service  RStudio. Contrairement à l'option `mem`, il ne s'agit pas d'un maximum mais d'un moment réservé, une valeur basse ne risque donc pas de bloquer vos traitements. Il est conseillé d'utiliser 2 comme valeur par défaut si vos traitements nécessitent de la parallélisation et laisser l'option par défaut sinon.
...
Son adresse est XXXX. Cependant, elle n'apporte pas un niveau de sécurité 
...
de détails). Les fonctionalités standards de RStudio pour favoriser l'usage
...
RStudio, il est recommandé de s'assurer que les codes (via Gitlab, voir ci-dessus)
et les données (via S3, voir ci-dessous) à conserver ont bien été
sauvegardés. 

```{r, echo = FALSE, fig.cap= "Accéder aux options d'un service", out.width = "50%"}
utilitr::include_image("./pics/SSPCloud/04_exemple_service.png")
```

En haut, se trouve une rangée de trois boutons utiles : ![](./pics/SSPCloud/04_exemple_service_kill.png#inline){width="15%"}\ :
...
* Le dernier bouton permet de tuer définitivement une session qui disparaîtra
...
Il est possible d'utiliser l'interface du `SSP Cloud` pour explorer les fichiers présents dans S3 ainsi qu'ajouter ou télécharger manuellement des fichiers. Le raccourci pour accéder aux fichiers disponibles se trouve dans la barre latérale, ![](./pics/SSPCloud/minio/onyxia_files.png#inline){width="30"}\
...
Voici la procédure à suivre pour renouveler les droits d'accès à S3. Cliquer sur `Mon compte` ![](./pics/SSPCloud/moncompte.png#inline){width="30"}\
...
```{r, echo = FALSE}
utilitr::include_image("./pics/SSPCloud/jetons_onyxia.png")
```
