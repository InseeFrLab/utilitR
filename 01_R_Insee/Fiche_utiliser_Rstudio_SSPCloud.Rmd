...
- [Cliquer sur l'onglet **"Catalogue de services"**](https://datalab.sspcloud.fr/my-lab/catalogue) qui est accessible à gauche grâce à ce symbole ![](./pics/SSPCloud/01_bouton_services.png){width="40"}
...
```{r lancer_rstudio, echo = FALSE, fig.cap= "Lancer un service RStudio"}
utilitr::include_image("./pics/SSPCloud/01_bouton_services.png", ratio_compression = 1)
```
...
```{r, echo = FALSE, fig.cap= "Lancer un service RStudio"}
...
```{r, echo = FALSE, fig.cap= "Page de création d'un service RStudio"}
...
Pour accéder à l'ensemble de ses services, il est nécessire de cliquer sur le bouton
suivant ![](./pics/SSPCloud/03_bouton_mes_services.png){width="40"}.
La page qui s'ouvre permet alors de lister l'ensemble des services ouverts:

```{r, echo = FALSE, fig.cap= "Liste des services ouverts"}
utilitr::include_image("./pics/SSPCloud/03_liste_services.png")
```

Pour accéder à l'un d'eux il faut cliquer sur le
bouton  ![](./pics/SSPCloud/Se_connecter_service.png){width="40"}
...
(![](./pics/SSPCloud/key.png){width="40"}) présente dans la page
*Mes services* ![](./pics/SSPCloud/03_bouton_mes_services.png){width="40"}.
Cette action permet de copier le mot de passe dans le presse-papier ce qui
facilite son utilisation.

Pour accéder à un service, le bouton ![](./pics/SSPCloud/Se_connecter_service.png){width="40"}
...
```{r, echo = FALSE, fig.cap= "Accéder aux options d'un service"}
utilitr::include_image("./pics/SSPCloud/04_exemple_service.png")
```

En haut, se trouvent trois boutons:

```{r, echo = FALSE}
utilitr::include_image("./pics/SSPCloud/04_exemple_service_kill.png")
```
...
stockés dans la session temporaire seront supprimés. Cela ne dispense donc
pas de sauvegarder ses codes via Gitlab et ses données via S3
