...
Cette commande peut toutefois renvoyer un grand nombre de résultats si le _bucket_ contient un grand nombre de fichiers. Le _bucket_ `donnees-insee` contient ainsi plus de 1000 fichiers. L'option `prefix` permet de spécifier le chemin des fichiers que l'on souhaite lister. Par exemple, le code suivant fournit la liste des fichiers situés dans le répertoire `Estel/2015` du _bucket_ `donnees.insee`:
