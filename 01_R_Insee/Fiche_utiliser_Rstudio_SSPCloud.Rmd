...
Imaginons qu'on souhaite accéder depuis un service Rstudio au _bucket_ nommé `donnees-insee` qui contient des fichiers de données d'exemples ([lien](https://datalab.sspcloud.fr/mes-fichiers/donnees-insee)). Le service Rstudio dispose déjà des droits d'accès au système S3, il n'y a donc pas besoin de les configurer. Pour lister l'ensemble des fichiers disponibles dans ce _bucket_, Il suffit de charger le _package_ `aws.s3` et d'exécuter la commande `get_bucket()`. Cette commande donne quelques informations sur les fichiers stockés dans le _bucket_: nom et emplacement, taille et date de dernière modification. 
...
::: {.conseil data-latex=""}
La commande `get_bucket()` peut renvoyer un grand nombre de résultats si le _bucket_ contient un grand nombre de fichiers. Le _bucket_ `donnees-insee` contient ainsi plus de 1000 fichiers. Il est donc conseille d'utiliser l'option `prefix` permet de spécifier le chemin des fichiers que l'on souhaite lister. Par exemple, le code suivant fournit la liste des fichiers situés dans le répertoire `Estel/2015` du _bucket_ `donnees.insee`:
