...
Même si certains lecteurs ont uniquement besoin de parcourir une fiche pour s'en imprégner, d'autres éprouveront le besoin d'exécuter des exemples simples de code pour se les approprier. C'est pourquoi la documentation `utilitR` propose un grand nombre d'exemples **reproductibles**. Cela signifie qu'en chargeant les _packages_ indiqués dans chaque fiche, le lecteur pourra travailler dans son environnement de travail sur les mêmes données que les exemples présentés, et reproduire le même résultat.
...
Dans la documentation `utilitR`, la reproductibilité de la plupart des exemples est assurée par l'utilisation du package `doremifasolData`.
...
```{r "contenu_doremifasolData2", echo = FALSE, results = "asis"}
tables <- data(package = "doremifasolData")$result
knitr::kable(
