...
La documentation `utilitR` est composée de fiches regroupées en deux grandes parties. La première partie explique comment utiliser `R` et `Rstudio` et les outils associés (`git`, Gitlab) dans les environnements informatiques proposés à l'Insee (AUSv3 et SSP Cloud). La seconde partie est constituée de fiches thématiques expliquant comment réaliser des tâches standards avec `R` (importation et manipulation de données, exploitation d'enquêtes, réalisation de graphiques, rédaction de documents...). Chaque fiche énonce des recommandations et conseils pour réaliser certaines tâches.
