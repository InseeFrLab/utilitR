...
Le _package_ `doremifasolData` n'est pas disponible sur le répertoire central des _packages_ `R` (le CRAN). La méthode pour installer le _package_ est légèrement différente en fonction du poste sur lequel vous travaillez.

Si vous utilisez `R` sur votre ordinateur personnel ou sur le SSP Cloud, il faut exécuter la commande suivante :
...
::: {.remarque}
Il peut arriver que vous rencontriez des difficultés à installer le _package_ `doremifasolData`. Si c'est le cas, n'hésitez pas à demander de l'aide, notamment aux référents du réseau LS2.
:::
