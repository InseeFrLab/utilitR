...
Selon les projets, les fichiers *environment* et *profile* peuvent avoir besoin d'être différents. Pour ce faire, `R` priorise le chargement de ces fichiers dans :l'ordre suivant 

1. Les fichiers présents dans un `Rproject` sont prioritaires.
2. A défault, les fichiers présents dans le _Home_ (sous Windows, ce sera typiquement _Mes documents_).
3. A défault, les fichiers présents dans le répertoire d'installation de `R`.

Attention, R ne travaille qu'avec un unique fichier *environment* et un unique fichier *profile*. Créer un nouveau fichier *environment* dans un projet désactivera donc l'éventuel fichier présent dans le _Home_. 
