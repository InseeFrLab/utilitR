...
* `read_csv()` importe par défaut toutes les colonnes du fichier. Il est possible mais vraiment complexe de sélectionner les colonnes qu'on veut importer (en utilisant la modalité `cols_only()` de l'option `col_types`).
Si vous avez besoin de sélectionner des colonnes, **il est fortement recommandé d'utiliser `fread()`** plutôt que `read_csv` (voir plus bas).
