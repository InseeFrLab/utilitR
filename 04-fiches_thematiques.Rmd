...
#### Fichiers de taille limitée:  `read_delim()` et `read_csv()`
...
* Si vous connaissez déjà `R`, vous pouvez utiliser directement les fonctions `read_csv()` et `read_delim()`.
:::
...
La fonction  `read_delim()` du *package* `readr` est faite pour lire toutes sortes de fichiers plats, et proposent un grand nombre d'options. La fonction `read_csv()` est une version simplifiée de `read_delim()`, adaptée au cas des fichiers `.csv`.
...
Dans l'exemple qui suit, on veut importer le fichier des communes du code officiel géographique, en déclarant que le fichier est encodé en UTF-8 et en imposant que le code commune soit lu comme une chaîne de caractères et le numéro de région comme un nombre entier, on écrit le code suivant: 
